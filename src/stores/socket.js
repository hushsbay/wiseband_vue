...
//각종 패널.vue를 제외한 윈도우 레이아웃은 1) Main.vue + MsgList.vue 2) MsgList.vue만 존재 3) Main.vue + MsgList.vue 방식 (사용자가 주소를 복사해 오픈한 경우) => 3가지임
//1)과 3)은 경합을 통해서 탭 하나만 데이터 폴링과 소켓을 소유함 (소켓은 브라우저별로 단 1개의 소켓만 접속하는 것을 원칙으로 함)
//소켓으로 데이터가 수신되면 데이터 폴링을 3초간 시작해라는 트리거만 제공할 뿐 다른 업무로직은 없음. 따라서, 송신시에도 별다른 업무로직은 없음
//브라우저에서 gst.realtime.emit()과 gst.realtime.onSock() 2개로 송수신하는데 Main.vue에는 송수신을 두고 나머지 .vue에서는 BroadcastChannel을 이용해 Main.vue로 전달해 송신하도록 함
//따라서, MsgList에서는 bc3 (Main.vue에 emit할 데이터를 전달하기 위한 전용 채널 : MsgList => Main) 만들어 사용 (각 패널에서는 소켓데이터 송수신 할 경우가 현재까지는 없음)