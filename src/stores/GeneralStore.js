...
const GeneralStore = defineStore('General', () => {
    ...
    const ctx = { 
        ...
        hide : function(e) { //Main.vue에서 @click시 click해도 메뉴가 안닫히도록 해야 눈에 보일 것임
            //아래에서 우클릭이 아닌 click에서 처리시 바로 닫히면 안되게 함
            if (e.srcElement.className.includes("maintainContextMenu")) return //에디터에서도 적용 (에디터내 <p>로 시작되면 srcElement이므로 이 경우는 parentElement로 체크)
            //if (e.srcElement.parentElement && e.srcElement.parentElement.className.includes("maintainContextMenu")) return
            this.on = false
            //const ele = document.elementFromPoint(e.clientX, e.clientY)
            //for () for loop로 맨 위까지 가면서 에디터 엘레먼트가 있는지 찾아 보기
            debugger
